...
groundhog.library(packages, groundhog.day, tolerate.R.version='4.2.0')
# you may need to...
...
drought_risk_continuous <- function(x) {
  x = (x - qt[[1]])/(qt[[6]] - qt[[1]]) * 20
  x
}
...
drought_r_continuous<- st_apply(drw, 1:2, drought_risk_continuous)


plot(drought_r)
plot(drought_r_continuous)


```
Visualize the distribution of the values of raster data of drought risk. 
```{r}
# Function to create a histogram in stars form
stars_histogram <- function(drw, n_breaks = 30, min_value = NULL, max_value = NULL) {
  if (is.null(min_value)) min_value <- min(drw, na.rm = TRUE)
  if (is.null(max_value)) max_value <- max(drw, na.rm = TRUE)
  
  breaks <- seq(min_value, max_value, length.out = n_breaks + 1)
  hist_data <- hist(drw, breaks = breaks, plot = FALSE)
  
  # Create stars object
  s <- st_as_stars(hist_data)
  st_set_dimensions(s)[[1]] <- st_dimension(s)
...
drw <- rnorm(1000)  # Replace with your own data
histogram_stars <- stars_histogram(drw)

# Plot the histogram in stars form
plot(histogram_stars)
...
vulnerability_r_cont <- (40 - capacity_r) + sensitivity_r + drought_r_continuous + flood_r 
# 100 = highest vulnerability, 0 = lowest vulnerability
names(vulnerability_r_cont) <- "vulnerability_drought"
plot(vulnerability_r_cont)
```
...
mean_ta_vuln <- aggregate(vulnerability_r, ta_v, FUN = mean) %>% 
  st_as_sf()

```

```{r}
# Map the vulnerability score by TA
breaks_vuln_ta <- classIntervals(mean_ta_vuln$vulnerability, 4, style = "jenks")$brks


tmap_mode("plot")
tm_shape(mean_ta_vuln)+
  tm_polygons(col = "vulnerability", breaks = breaks_vuln_ta, title = "Malawi Vulnerability Score (TA)", palette = "OrRd", lwd = 0.1, border.col = "grey")+
  tm_layout(main.title = "Malawi Vulnerability Score", main.title.size = 0.9, main.title.position = "center", frame = FALSE)+
  tm_layout(legend.width=2, legend.text.size = .3, legend.title.size = 0.5, asp=0.8)
...
After creating the mean vulnerability score for each TA in our reproduction, I decide also to create mean vulnerability score for each TA using the data from the original study. 

```{r original-Fig5-TA-Vulnerability-mapping1}
# take mean vulnerability score across TA
mean_ta_vuln_orig <- aggregate(orfig5rast, ta_v, FUN = mean) %>% 
  st_as_sf()

```

```{r original-Fig5-TA-Vulnerability-mapping2}
# Map the vulnerability score by TA
breaks_vuln_ta_orig <- classIntervals(mean_ta_vuln_orig$orv, 4, style = "jenks")$brks


tmap_mode("plot")
tm_shape(mean_ta_vuln_orig)+
  tm_polygons(col = "orv", breaks = breaks_vuln_ta_orig, title = "Malawi Vulnerability Score (TA)", palette = "OrRd", lwd = 0.1, border.col = "grey")+
  tm_layout(main.title = "Malawi Vulnerability Score (Original)", main.title.size = 0.9, main.title.position = "center", frame = FALSE)+
  tm_layout(legend.width=2, legend.text.size = .3, legend.title.size = 0.5, asp=0.8)



comparison_ta_vul <- st_join(mean_ta_vuln_orig, mean_ta_vuln, join=st_intersects) %>%
  mutate(diff = orv.x - vulnerability)

tmap_mode("plot")
tm_shape(comparison_ta_vul)+
  tm_polygons(col = "diff", title = "Original - Reproduction", palette = "BrBG", lwd = 0.1, border.col = "grey")+
  tm_layout(main.title = "Malawi Vulnerability Score Comparison", main.title.size = 0.9, main.title.position = "center", frame = FALSE)+
  tm_layout(legend.width=2, legend.text.size = .3, legend.title.size = 0.5, asp=0.8)
